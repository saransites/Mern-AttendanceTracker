...
const Login = ({ handlesignup }) => {
  ...
  const gotolandingpage = (e) => {
    ...
    signInWithEmailAndPassword(auth, email, password)
      .then((user) => {
        if (user.user.uid == 'Om78tMKulZchnbbRvBXY0lK0aZp1') {
          navigate("/adminpage")
        }else{
      ...
      <div className="p-8 max-w-md w-full">
        ...
        <form onSubmit={gotolandingpage} className="space-y-4">
          ...
          <div>
            <button
              type="submit"
              className="w-full bg-[#82729255] py-3 px-4 hover:bg-[#82729296] rounded-md text-white font-semibold transition duration-300 ease-in-out hover:from-pink-600 hover:to-purple-600 focus:outline-none"
            >
              Login
            </button>
          </div>
